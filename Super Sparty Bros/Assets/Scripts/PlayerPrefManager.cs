...
public static class PlayerPrefManager {
	...
	public static bool GetBigness() {
		if (PlayerPrefs.HasKey("Bigness")) {
			if (PlayerPrefs.GetInt("Bigness") == 1) {
				return true;
			} else {
				return false;
			}
	...
	public static void SetBigness(bool bignessKey) {
		PlayerPrefs.SetInt("Bigness", bignessKey ? 1 : 0);
	}


	// story the current player state info into PlayerPrefs
	public static void SavePlayerState(int score, int highScore, int lives, bool isBig) {
		// save currentscore and lives to PlayerPrefs for moving to next level
		PlayerPrefs.SetInt("Score",score);
		PlayerPrefs.SetInt("Lives",lives);
		PlayerPrefs.SetInt("Highscore",highScore);
		PlayerPrefs.SetInt("Bigness", isBig ? 1 : 0);
	}
