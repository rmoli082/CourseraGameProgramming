...
public class CharacterController2D : MonoBehaviour {
	...
	void Awake () {
		...
		_icyLayer = LayerMask.NameToLayer("IcyGround");
	}
	...
	void Update()
	{
		...
		// allow double jump after grounding
		if (_isGrounded) {
			_canDoubleJump = true;
		}  
		...
		{
			DoJump();
		} else if (_canDoubleJump && Input.GetButtonDown("Jump")) // If can double jump, and jump pressed, allow double jump
		{
			DoJump();
		...
		if (_touchingIce)
			_rigidbody.velocity = new Vector2(_vx * moveSpeed * 5f, _vy);
		else
		...
		Physics2D.IgnoreLayerCollision(_playerLayer, _platformLayer, (_vy > 0.0f || Input.GetAxisRaw("Vertical") < 0.0f));
	...
	public void EnemyStun() {
		if (!_facingRight)
			_rigidbody.AddForce(new Vector2(_vx*moveSpeed*400f, 0));
		else 
			_rigidbody.AddForce(new Vector2(-400f, 0));
	}

	public void GrowBigger() {
		GameManager.gm.isBig = true;
		playerHealth = 10;
	}

	public void ShrinkPlayer() {
		GameManager.gm.isBig = false;
	}
}
